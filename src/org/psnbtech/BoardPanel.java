...
public class BoardPanel extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		/*
		 * Draw the grid on the board. This makes it easier to see exactly where we in
		 * relation to the fruit.
		 * 
		 * The panel is one pixel too small to draw the bottom and right outlines, so we
		 * outline the board with a rectangle separately.
		 */
	...
	private void drawTile(int x, int y, TileType type, Graphics g) {
		/*
		 * Because each type of tile is drawn differently, it's easiest to just run
		 * through a switch statement rather than come up with some overly complex code
		 * to handle everything.
		 */
		switch (type) {

		/*
		 * A fruit is depicted as a small red circle that with a bit of padding on each
		 * side.
		 */
		case Fruit:
		...
		/*
		 * The snake body is depicted as a green square that takes up the entire tile.
		 */
		case SnakeBody:
		...
		/*
		 * The snake head is depicted similarly to the body, but with two lines
		 * (representing eyes) that indicate it's direction.
		 */
		case SnakeHead:
			 ...
			 * 
			 * Vertical lines indicate that it's facing North or South, and Horizontal lines
			 * indicate that it's facing East or West.
			 * 
			 ...
			 * 
			 * First, we add (or subtract) EYE_SMALL_INSET to or from the side of the tile
			 * representing the direction we're facing. This will be constant for both eyes,
			 * and is represented by the variable 'baseX' or 'baseY' (depending on
			 * orientation).
			 * 
			 * Next, we add (or subtract) EYE_LARGE_INSET to and from the two neighboring
			 * directions (Example; East and West if we're facing north).
			 * 
