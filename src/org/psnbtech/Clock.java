...
public class Clock {
	 ...
	 * the last update time will be reset to the current time, and the paused flag
	...
	/**
	 * Updates the clock stats. The number of elapsed cycles, as well as the cycle
	 * excess will be calculated only if the clock is not paused. This method should
	 * be called every frame even when paused to prevent any nasty surprises with
	 * the delta time.
	 */
	public void update() {
	...
	/**
	 * Pauses or unpauses the clock. While paused, a clock will not update elapsed
	 * cycles or cycle excess, though the {@code update} method should still be
	 * called every frame to prevent issues.
	 * 
	...
	/**
	 * Checks to see if a cycle has elapsed for this clock yet. If so, the number of
	 * elapsed cycles will be decremented by one.
	 * 
	 ...
	 * {@code hasElapsedCycle}, the number of cycles will not be decremented if the
	 * number of elapsed cycles is greater than 0.
	 * 
	 * @return Whether or not a cycle has elapsed.
	 * @see hasElapsedCycle
	 */
