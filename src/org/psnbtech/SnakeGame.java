...
public class SnakeGame extends JFrame {
	...
	/**
	 * The minimum length of the snake. This allows the snake to grow right when the
	 * game starts, so that we're not just a head moving around on the board.
	 */
	...
	public SnakeGame() {
		...
		addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				/*
				 * If the game is not over, toggle the paused flag and update the logicTimer's
				 * pause flag accordingly.
				 */
				case KeyEvent.VK_P:
					if (!isGameOver) {
						isPaused = !isPaused;
		...
		/*
		 * Resize the window to the appropriate size, center it on the screen and
		 * display it.
		 */
		pack();
	...
	private void startGame() {
		...
		/*
		 * This is the game loop. It will update and render the game and will continue
		 * to run until the game window is closed.
		 */
		while (true) {
			...
			/*
			 * Calculate the delta time between since the start of the frame and sleep for
			 * the excess time to cap the frame rate. While not incredibly accurate, it is
	...
	private void updateGame() {
		/*
		 * Gets the type of tile that the head of the snake collided with. If the snake
		 * hit a wall, SnakeBody will be returned, as both conditions are handled
		 * identically.
		 */
		 ...
		 * 
		 * Fruit: If we collided with a fruit, we increment the number of fruits that
		 * we've eaten, update the score, and spawn a new fruit.
		 * 
		 * SnakeBody: If we collided with our tail (or a wall), we flag that the game is
		 * over and pause the game.
		 * 
		 * If no collision occurred, we simply decrement the number of points that the
		 * next fruit will give us if it's high enough. This adds a bit of skill to the
		 * game as collecting fruits more quickly will yield a higher score.
		 */
	...
	private TileType updateSnake() {
		...
		/*
		 * Here we peek at the next direction rather than polling it. While not game
		 * breaking, polling the direction here causes a small bug where the snake's
		 * direction will change after a game over (though it will not move).
		 */
		Direction direction = directions.peekFirst();

		/*
		 * Here we calculate the new point that the snake's head will be at after the
		 * update.
		 */
		...
		/*
		 * If the snake has moved out of bounds ('hit' a wall), we can just return that
		 * it's collided with itself, as both cases are handled identically.
		 */
		...
		/*
		 * Here we get the tile that was located at the new head position and remove the
		 * tail from of the snake and the board if the snake is long enough, and the
		 * tile it moved onto is not a fruit.
		 * 
		 * If the tail was removed, we need to retrieve the old tile again incase the
		 * tile we hit was the tail piece that was just removed to prevent a false game
		 * over.
		 */
		...
		/*
		 * Update the snake's position on the board if we didn't collide with our tail:
		 * 
		 * 1. Set the old head position to a body tile. 2. Add the new head to the
		 * snake. 3. Set the new head position to a head tile.
		 * 
	...
	private void resetGame() {
		/*
		 * Reset the score statistics. (Note that nextFruitPoints is reset in the
		 * spawnFruit function later on).
		 */
		this.score = 0;
		this.fruitsEaten = 0;
	...
	private void spawnFruit() {
		...
		/*
		 * While we could just as easily choose a random index on the board and check it
		 * if it's free until we find an empty one, that method tends to hang if the
		 * snake becomes very large.
		 * 
		 * This method simply loops through until it finds the nth free index and
		 * selects uses that. This means that the game will be able to locate an index
		 * at a relatively constant rate regardless of the size of the snake.
		 */
		int freeFound = -1;
